...
##### QUE SALTE A LA RUTINA DE SERVICIO CORRESPONDIENTE
		beq	$v0, WRITE_D7SEG, WRITE_D7SEG_RTE
...
CHECK:
	beq	$a1, 1, WRITE_LEFT_RTE # Si a1 = 1, saltar a rutina escribir en izquierdo
	beq	$a1, $zero, WRITE_LEFT_RTE # Si a1 = 0, saltar a rutina escribir en derecho
	j	CONTINUE_RTE # otro, dato erroneo, salir de rutina
WRITE_LEFT_RTE: # Visualizador izquierdo
	la	$t0, 0xffff0011
	jal	cod7seg
	li	$t1, $v0
	sb	$t1, 0($t0)
	j	CONTINUE_RTE
WRITE_RIGHT_RTE: # Visualizador derecho
	la	$t0, 0xffff0010
	jal	cod7seg
	li	$t1, $v0
	sb	$t1, 0($t0)
